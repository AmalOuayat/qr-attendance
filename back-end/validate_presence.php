...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $now = new DateTime();
        $date_seance = new DateTime($seance['date_debut']);
        $date_seance_fin = new DateTime($seance['date_fin']);

        // Initialiser le statut par défaut
        $status = 'absent';
        $gps_required = true;

        // Vérifier si la séance est en cours ou terminée
        if ($now >= $date_seance && $now <= $date_seance_fin) {
            ...
            // Vérifier la géolocalisation seulement si la séance est en cours
            ...
            // Vérification de la distance (50m max pour la démo)
            $distanceSeuil = 50; // 50 mètres
            ...
            // Séance terminée - marquer absent automatiquement
            $gps_required = false;
            $status = 'absent';
        } else {
